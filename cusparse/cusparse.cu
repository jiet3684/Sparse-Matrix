...
int main(int argc, char *argv[])
{
    ...
    cusp::io::read_matrix_market_file(B, argv[3]);

//    B = A;



    double t_prespgemm=0;
    double t_spgemm=0;
...
#pragma omp parallel for
        ...
        std::cout << "# elements : " << nnzA << std::endl;
        ...
        cudaMemcpy(h_rC , d_rC, sizeof(int) * (n + 1), cudaMemcpyDeviceToHost);  
        ...
        fclose(fp);*/
        t_prespgemm += pre_spgemm.Elapsed();
        t_spgemm += spgemm.Elapsed();
        free(h_C);
        free(h_cC);
        free(h_rC);

    }
    printf("%s pre spgemm : %lf spgemm %lf total %lf %d\n",argv[2], t_prespgemm/TRY, t_spgemm/TRY, (t_prespgemm+t_spgemm)/TRY,nnzC);
    gpu_to_host_C.Start();
    gpu_to_host_C.Stop();
